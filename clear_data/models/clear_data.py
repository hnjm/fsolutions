...
class ClearDataModel(models.Model):
    ...
    internal_model_list = fields.Char(string='Always clear',
                                      descripton="Model in this list do not need installed. style like this:[string,string,...]")
    model_clear_ids = fields.Many2many("ir.model", 'clear_data_ir_model_rel', 'group_id', 'model_id',
                                       domain=[('model', 'not in', ['clear.data.model'])])
    ...
    def _do_clear_by_model_name(self, model_name):
        ...
        if model_obj == None or not odoo.tools.table_exists(self._cr, model_obj._table):
            return
        self._cr.execute("""DELETE FROM """+model_obj._table+ """ WHERE create_date > %s AND create_date < %s""", [self.date_from, self.date_to])
